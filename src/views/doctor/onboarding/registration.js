...
import {EmailOutlined, PhoneEnabledOutlined, Visibility, VisibilityOff} from "@mui/icons-material";
...
export default function Registration({userDetails, userReg, updatePage}) {
    ...
    return (
        <Box sx={{mt: {xs: 2, sm: 10}, minHeight: {xs: '90vh', sm: '580px'}, padding: {xs: '20px', sm: '50px'}}}
             className={'onboarding-content-box'}>

            {
                stage === 1
                    ? <Box>
                        <h6 className={'onboarding-title'}>Whats your Primary email? <span
                            style={{color: '#FF0202'}}>*</span></h6>
                        <h6 className={'onboarding-subtitle'}>We will only use it to advise you for any important
                            changes.</h6>
                    ...
                    : <Box>
                        <h6 className={'onboarding-title'}>Enter 4-digit code sent to your email.</h6>
                        <h6 className={'onboarding-subtitle'}> We’ve sent it to {userDetails?.email || ''}</h6>
                        <Box sx={{
                        ...
                        <Box sx={{marginTop: {xs: '25%', sm: '38%'}}}>
                            <Button onClick={() => updatePage(2)} disableElevation type={"submit"}
                                    sx={{padding: '13px 45px', textTransform: 'none'}} variant={'contained'}>
                                Continue
                            </Button>
                        </Box>
