...
export default function Index() {
    ...
    return (
        ...
        <Box className={"main-wrapper"}>
            ...
            <MobileSidebar location={location} toggleNav={setMobileNav} mobileNav={mobileNav}/>
            <Toolbar sx={{mt: 2}}/>
            <Grid container spacing={3} className={'content container-fluid'}>
                <Grid item xs={12} sm={3} lg={3} md={3} sx={{
                    display: {
                        xs: 'none',
                        sm: !checkPath(sidePath) ? 'block' : 'none'
                    }
                }}>
                    <Sidebar location={pathname} mobileNav={mobileNav} toggleNav={setMobileNav}/>

                </Grid>

                <Grid item xs={12} sm={!checkPath(sidePath) ? 9 : 12} lg={!checkPath(sidePath) ? 9 : 12}
