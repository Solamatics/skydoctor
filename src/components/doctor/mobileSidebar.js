                    ...
                    <List sx={{fontSize: '11px!important'}}>
                        {
                            routes.map(item => (
                                <ListItem  className={'sidebar-nav-list sidebar-nav'} key={item.title}>
                                    <ListItemButton onClick={()=>toggleNav(false)} component={Link} to={item.route}>
                                        <ListItemIcon className={'sidebar-nav-icon'}>
                                            <Icon sx={{color:item.route === location ?'#0de0fe' :'#757575'}}>{item.icon} </Icon>
                                        </ListItemIcon>
                                        <ListItemText>
                                            <span style={{fontSize: '11px!important', color:item.route === location ?'#0de0fe' :'#757575'}}>
                                                {item.title}
                                            </span>
                                        </ListItemText>
                                    </ListItemButton>
